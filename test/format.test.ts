...
test("prompt asks Codex to use native send actions for local media", () => {
  ...
  assert.match(prompt, /wemo-actions/);
...
test("prompt tells Codex to inspect inbound attachment paths", () => {
  const prompt = buildPrompt("analyze this voice", [{
    kind: "audio",
    label: "voice.silk",
    path: "C:/Users/THU/.wemo/inbound/voice.silk"
  }]);

  assert.match(prompt, /WeChat audio: voice\.silk saved to C:\/Users\/THU\/\.wemo\/inbound\/voice\.silk/);
