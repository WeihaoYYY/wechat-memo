...
| ✅ | WeChat private-chat control | Supports regular messages plus `/status`, `/new`, `/resume`, `/bind`, `/model`, `/effort`, `/prompt start`, `/prompt done`, and `/stop`. `/stop` stays responsive during long tasks, overlapping ordinary tasks from one sender are rejected safely, and an idle interrupted App Server is reset so Windows command children do not linger. | Pending: `docs/images/screenshots/wechat-chat.png` |
| ✅ | WeChat media input | Accepts voice transcription explicitly labeled for Codex, plus images, audio, video, and files up to 100 MiB each, with a direct notice when the limit is exceeded. | Pending: `docs/images/screenshots/wechat-media-input.png` |
